...
class DLA_BRAMAC(object):
    def __init__(self, qvec=2, cvec=12, kvec=60, rvec=3, svec=1, lw=1, lh=1,
        ...
        if self.qvec == 4:
            ...
            raise Exception('ERROR! Qvec can only be 3 or 4 !!!')
    ...
    def count_streambuf_bram(self, cin, pin, qin, cout, pout, qout, network_type='alexnet'):
        ...
        bram_depth  = self.num_bram_row * (self.num_bram_col / self.bram_dwidth) * (32 / self.precision)
        return math.ceil( ifmap_depth / bram_depth ) * num_bank

    def count_fcache_bram(self):
        return math.ceil(self.bramac_cvec_per_iter / self.bramac_cvec_per_bram) * self.rvec * \
...
with open('bramac_2sa_' + nn_type + '_speedup.txt', 'w') as f:
